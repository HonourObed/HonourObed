# Hi, I’m Honour 👋

<p align="left">
  <a href="https://www.linkedin.com/in/obed-honour-eje-atomic/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://discord.com/users/honourobed" target="_blank">
    <img src="https://img.shields.io/badge/Discord-5865F2?style=flat&logo=discord&logoColor=white" />
  </a>
</p>


**Undergraduate Research Assistant** and **Mechatronics Engineering student** based in Nigeria 🇳🇬,  
with a strong focus on **intelligent robotic systems, control theory, and applied AI**.

I’m particularly interested in building and simulating robotic systems that integrate sensing, control, and decision-making across mechanical, electrical, and computational domains.

---

## 🔬 Current Focus
- **Robotics Simulation** (modeling and testing systems before deployment)
- **Intelligent Robotics & Control Systems**
- **Human–Robot Interaction** (research interest; actively developing foundations)

---

## 🧠 Research & Technical Interests
- Autonomous & Intelligent Robotic Systems  
- Control Theory, Optimization, and System Modeling  
- Applied Artificial Intelligence in Robotics  
- Human–Robot Interaction  
- Engineering solutions with real-world impact

---

## 🛠 Tools & Languages
<img src="https://skillicons.dev/icons?i=cpp,python,matlab,arduino,linux,ros" />


---

## 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=transparent)

![GitHub Streak](https://streak-stats.demolab.com?user=YOUR_USERNAME&theme=transparent)

---

## 📌 Notes
- This GitHub documents learning, experimentation, and research-oriented projects  
- Repositories include simulations, control systems work, robotics experiments, and applied programming  
- Some interests may not yet be fully reflected in repositories — actively evolving


